def _find_vertex_to_color_dsatur(
    graph, not_colored_vertices, adjacent_vertices_colors
):
    vertex_to_color = next(iter(not_colored_vertices))
    for vertex in not_colored_vertices:
        greater_saturation = len(adjacent_vertices_colors[vertex]) > len(
            adjacent_vertices_colors[vertex_to_color]
        )
        equal_saturation = len(adjacent_vertices_colors[vertex]) == len(
            adjacent_vertices_colors[vertex_to_color]
        )
        greater_degree = graph.get_vertex_degree(
            vertex
        ) > graph.get_vertex_degree(vertex_to_color)
...
def dsatur(graph):
    ...
    while len(not_colored_vertices) != 0:
        vertex_to_color = _find_vertex_to_color_dsatur(
            graph, not_colored_vertices, adjacent_vertices_colors
        )
        color = min(
            set(range(used_colors_length + 1))
            - adjacent_vertices_colors[vertex_to_color]
        )
