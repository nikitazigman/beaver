...
class StableMatch:
    ...
    def get_stable_pairs_by_estimations(self):
        ...
        pairs = dict(
            [(user, None) for user in range(len(self.task_preferences_matrix))]
        )
        while len(tasks) > 0:
            current_task = self.get_current_task(tasks)
            current_task_users_estimations = (
                self.get_current_task_users_estimations(current_task)
            )
            for estimation, user in current_task_users_estimations:
                ...
                if task is None:
                    ...
                    preference_for_task = self.task_preferences_matrix[
                        user
                    ].index(task)
                    preference_for_current_task = self.task_preferences_matrix[
                        user
                    ].index(current_task)
    ...
    def get_stable_pairs_by_preferences(self):
        ...
        while len(tasks) > 0:
            current_task = self.get_current_task(tasks)
            current_task_users_priorities = (
                self.get_current_task_users_priorities(current_task)
            )
            for priority, user in current_task_users_priorities:
                ...
                if task is None:
                    ...
                    estimation_for_task = self.task_estimations_matrix[task][
                        user
                    ]
                    estimation_for_current_task = self.task_estimations_matrix[
                        current_task
                    ][user]
    ...
    def get_current_task_users_estimations(self, current_task):
        ...
        users_estimations_array = [
            (self.task_estimations_matrix[current_task][i], i)
    ...
    def get_current_task_users_priorities(self, current_task):
        ...
        users_priorities_array = [
            (self.task_preferences_matrix[i].index(current_task), i)
            for i in range(arr_size)
        ]
