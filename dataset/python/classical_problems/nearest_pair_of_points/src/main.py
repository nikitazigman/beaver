...
def nearest_points(points):
    ...
    def nearest_points_helper(points):
        if len(points) <= 3:
            return brute_force(points), sorted(
        ...
        merged_parts = list(
            merge(left_sorted, right_sorted, key=lambda point: point[1])
        )
        filtered_points = [
            point
            for point in merged_parts
            if left_points[-1][0] - point[0] < min_distance
        ]
...
def brute_force(points):
    ...
    for i in range(len(points)):
        for j in range(i + 1, len(points)):
            min_distance = min(
                min_distance, euclidian_distance(points[i], points[j])
            )
    return min_distance
